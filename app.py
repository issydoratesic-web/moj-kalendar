...
if any("Brow lift" in u or "laminacija" in u.lower() for u in odabrane_usluge):
    lam_da_ne = st.radio("Laminacija u zadnjih 6 tj?", ["Da", "Ne"], index=None, key="lam_q")
    alergije = st.text_input("Alergije?", key="alg_q")

novi_klijent = st.radio("Novi klijent?", ["Da", "Ne"], index=None, key="novi_q")
